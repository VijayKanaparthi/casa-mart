...
class LivingRoomItems extends Component {
  state = { livingroomData: [], status: apiStatus.initial, search: "" }

  componentDidMount() {
    this.getLivingRoomData()
  }
  getLivingRoomData = async () => {
  ...
  filterPrice = async (price) => {
    const { livingroomData } = this.state
    if (price <= 10000) {
      await this.getLivingRoomData()
      const filterHomeAppilencesData = livingroomData.filter(
        (eachProduct) => eachProduct.price <= price
      )
      this.setState({ livingroomData: filterHomeAppilencesData })
    } else if (price <= 30000) {
      await this.getLivingRoomData()
      const filterHomeAppilencesData = livingroomData.filter(
        (eachProduct) => eachProduct.price <= price
    ...
    return (
      <div className="w-full">
        <ul className="flex justify-center items-center md:justify-start md:items-start md:p-3 flex-wrap">
          {livingroomData.length > 0 ? (
            livingroomData.map((each) => (
              <Link to={`/livingroomitems/${each.id}`}>
                <li
                  key={each.id}
  ...
  searchBasedFilter = () => {
    const { livingroomData, search } = this.state

    if (search.length === 0) {
      this.getLivingRoomData()
    } else {
      const searchFilter = livingroomData.filter((product) =>
        product.name.toLowerCase().includes(search.toLowerCase())
      )
      this.setState({ livingroomData: searchFilter })
    }
  }
                  ...
                  <div className="cursor-pointer">
                    ...
                    <div>
                      <button
                        ...
                        onClick={this.getLivingRoomData}
                      >
                        Clear Filter
                      </button>
                    </div>
