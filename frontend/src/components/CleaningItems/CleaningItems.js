...
class CleaningItems extends Component {
  state = { cleaningItemsData: [], status: apiStatus.initial, search: "" }

  componentDidMount() {
    this.getCleaningItems()
  }
  getCleaningItems = async () => {
    ...
    if (response.ok === true) {
      ...
      this.setState({
        cleaningItemsData: responseData,
  ...
  filterPrice = async (price) => {
    const { cleaningItemsData } = this.state
    if (price <= 10000) {
      await this.getCleaningItems()
      const filterHomeAppilencesData = cleaningItemsData.filter(
        (eachProduct) => eachProduct.price <= price
      )
      this.setState({ cleaningItemsData: filterHomeAppilencesData })
    } else if (price <= 30000) {
      await this.getCleaningItems()
      const filterHomeAppilencesData = cleaningItemsData.filter(
        (eachProduct) => eachProduct.price <= price
    ...
    return (
      <div className="w-full">
        <ul className="flex justify-center items-center md:justify-start md:items-start md:p-3 flex-wrap">
          {cleaningItemsData.length > 0 ? (
            cleaningItemsData.map((each) => (
              <Link to={`/cleaningItems/${each.id}`}>
                <li
                  key={each.id}
  ...
  searchBasedFilter = () => {
    const { cleaningItemsData, search } = this.state

    if (search.length === 0) {
      this.getCleaningItems()
    } else {
      const searchFilter = cleaningItemsData.filter((product) =>
        product.name.toLowerCase().includes(search.toLowerCase())
      )
      this.setState({ cleaningItemsData: searchFilter })
    }
  }
