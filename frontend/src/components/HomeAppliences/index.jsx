...
class HomeAppliences extends Component {
  state = { homeAppliencesData: [], status: apiStatus.initial, search: "" }

  componentDidMount() {
    this.getHomeAppliencesData()
  }
  getHomeAppliencesData = async () => {
    ...
    if (response.ok === true) {
      ...
      this.setState({
        homeAppliencesData: responseData,
  ...
  filterPrice = async (price) => {
    const { homeAppliencesData } = this.state
    if (price <= 10000) {
      await this.getHomeAppliencesData()
      const filterHomeAppilencesData = homeAppliencesData.filter(
        (eachProduct) => eachProduct.price <= price
      )
      this.setState({ homeAppliencesData: filterHomeAppilencesData })
    } else if (price <= 30000) {
      await this.getHomeAppliencesData()
      const filterHomeAppilencesData = homeAppliencesData.filter(
        (eachProduct) => eachProduct.price <= price
    ...
    return (
      <div className="w-full">
        <ul className="flex justify-center items-center md:justify-start md:items-start md:p-3 flex-wrap">
          {homeAppliencesData.length > 0 ? (
            homeAppliencesData.map((each) => (
              <Link to={`/GetHomeAppliances/${each.id}`}>
                <li
                  key={each.id}
  ...
  searchBasedFilter = () => {
    const { homeAppliencesData, search } = this.state

    if (search.length === 0) {
      this.getHomeAppliencesData()
    } else {
      const searchFilter = homeAppliencesData.filter((product) =>
        product.name.toLowerCase().includes(search.toLowerCase())
      )
      this.setState({ homeAppliencesData: searchFilter })
    }
  }
                  ...
                  <div className="cursor-pointer">
                    ...
                    <div>
                      <button
                        ...
                        onClick={this.getHomeAppliencesData}
                      >
                        Clear Filter
                      </button>
                    </div>
