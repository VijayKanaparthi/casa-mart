...
class FurnitureItems extends Component {
  state = { furnitureData: [], status: apiStatus.initial, search: "" }

  componentDidMount() {
    this.getFurnitureItems()
  }
  getFurnitureItems = async () => {
  ...
  filterPrice = async (price) => {
    ...
    if (price <= 10000) {
      await this.getFurnitureItems()
      const filterHomeAppilencesData = furnitureData.filter(
        (eachProduct) => eachProduct.price <= price
      )
      this.setState({ furnitureData: filterHomeAppilencesData })
    } else if (price <= 30000) {
      await this.getFurnitureItems()
      const filterHomeAppilencesData = furnitureData.filter(
        (eachProduct) => eachProduct.price <= price
        ...
        <ul className="flex justify-center items-center md:justify-start md:items-start md:p-3 flex-wrap">
          {furnitureData.length > 0 ? (
            furnitureData.map((each) => (
              <Link to={`/getfurnitureitems/${each.id}`}>
                <li
                  key={each.id}
  ...
  searchBasedFilter = () => {
    const { furnitureData, search } = this.state

    if (search.length === 0) {
      this.getFurnitureItems()
    } else {
      const searchFilter = furnitureData.filter((product) =>
        product.name.toLowerCase().includes(search.toLowerCase())
      )
      this.setState({ furnitureData: searchFilter })
    }
  }
                  ...
                  <div className="cursor-pointer">
                    ...
                    <div>
                      <button
                        ...
                        onClick={this.getFurnitureItems}
                      >
                        Clear Filter
                      </button>
                    </div>
